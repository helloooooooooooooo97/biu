...
<p>
  <em>“麦克卢汉说，媒介即讯息。<br />
  我们重构了终极媒介，因而定义了终极讯息。<br />
  与其问 BIU 能做什么，不如想想 BIU 不能做什么。”</em>
</p>
...
正如毕加索所言：“优秀的艺术家模仿，伟大的艺术家剽窃。”Biu 不照搬功能表象，而是从以下四款经典系统中汲取底层模型，重组为自己的核心底座：
...
页面正文在 SQLite 里，附件统一进 `.biu/assets`：

```
.biu/pages.sqlite     ← 标题、正文 notes、树与合集
.biu/assets/          ← 图片、画板 JSON 等附件（全库共用）
```
...
页面正文存放在 `.biu/pages.sqlite`，附件统一在 `.biu/assets`；表记录则保存在 File System 的存储中。每项能力可以选择适合自己的底层存储，但都按照同一份接口契约暴露在同一棵树上。
...
macOS / Windows 安装包在 [GitHub Releases](https://github.com/helloooooooooooooo97/biu/releases)。macOS 为 ad-hoc 签名：拖进「应用程序」后若提示已损坏，终端执行一行即可：

```bash
xattr -dr com.apple.quarantine /Applications/Biu.app
```

说明见 [docs/desktop-install.md](docs/desktop-install.md)。
