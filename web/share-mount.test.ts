...
test('share mount picks content chrome by collection', () => {
  ...
  assert.match(chrome, /installShareCollectionChrome/)
  assert.match(chrome, /decorate\('\/sessions'/)
  assert.match(chrome, /decorate\('\/skills'/)
  assert.match(chrome, /decorate\('\/plugins'/)
  assert.match(chrome, /decorate\('\/tasks'/)
  assert.match(chrome, /DEFAULT_CHROME_PATH/)
  assert.match(chrome, /getDatabaseUi\(\)\?\.chrome\(collection\)/)
  assert.doesNotMatch(chrome, /collection === '\/sessions'/)
  const plugins = readFileSync(resolve(import.meta.dirname, './share-plugins.ts'), 'utf8')
  assert.match(plugins, /installShareCollectionChrome/)
  assert.match(plugins, /databaseUi/)
})
...
test('share mount loads page plugins without booting the workstation', () => {
  ...
  const page = readFileSync(resolve(import.meta.dirname, '../packages/host-share/src/web/share-page.tsx'), 'utf8')
  ...
  assert.match(page, /LayoutPrefsMenu/)
  assert.match(page, /persistPagePrefs|getPagePrefs/)
  ...
  const table = readFileSync(resolve(import.meta.dirname, '../packages/host-share/src/web/share-table.tsx'), 'utf8')
  assert.match(table, /tasks-table-stage/)
  assert.match(table, /fsdb-check-rail/)
  const bar = readFileSync(resolve(import.meta.dirname, '../packages/host-share/src/web/share-view-bar.tsx'), 'utf8')
  ...
  const popover = readFileSync(resolve(import.meta.dirname, '../packages/host-share/src/web/share-popover.tsx'), 'utf8')
