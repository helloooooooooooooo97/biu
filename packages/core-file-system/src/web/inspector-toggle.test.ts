...
test('embedded record details can share their explicit record target', () => {
  ...
  assert.match(detail, /shareLabel=\{collectionPath === '\/pages' \? '分享页面' : '分享记录'\}/)
  assert.match(detail, /data-testid="fsdb-detail-share"/)
...
test('sidebar records paint detail immediately without reloading the view', () => {
  ...
  assert.match(css, /\.fsdb-emoji-picker \{[^}]*background:\s*var\(--dsw-bubble\)/s)
...
test('database extras sit after the record detail, not in the inspector', () => {
  ...
  assert.match(style, /\.heading-outline-host\{[^}]*width:64px/)
  assert.match(style, /\.heading-outline-host \.chat-outline::before\{[^}]*width:64px/)
  assert.doesNotMatch(style, /\.heading-outline-host \.chat-outline\{[^}]*pointer-events:auto/)
  assert.doesNotMatch(style, /\.heading-outline-host:has\(\.chat-outline:hover\)\{[^}]*width:360px/)
  assert.match(style, /\.fsdb-detail-float-nav:hover,\.fsdb-detail-float-nav:focus-within,\.fsdb-detail-float-nav:has\(\[aria-expanded=true\]\),html\[data-nav-pin="1"\] \.fsdb-detail-float-nav\{[^}]*opacity:1/)
  ...
  assert.match(style, /\.fsdb-main > \.fsdb-detail-title-row,\.fsdb-main > \.fsdb-detail-icon-slot\{[^}]*flex:none/)
  ...
  assert.match(style, /\.fsdb-detail-title\{[^}]*max-height:none/)
  assert.match(style, /\.fsdb-detail-title-input\{[^}]*field-sizing:content/)
  assert.match(style, /\.fsdb-detail-title-input\{[^}]*max-height:none/)
  assert.match(detail, /autoSize/)
  assert.doesNotMatch(detail, /length > 48/)
  ...
  assert.doesNotMatch(style, /\.fsdb-banner-story\{[^}]*top:52px/)
  assert.doesNotMatch(style, /\.fsdb-banner-title-actions\{[^}]*top:calc\(100%/)
  assert.doesNotMatch(style, /\.fsdb-detail-title-row:hover \.fsdb-banner-title-actions/)
  assert.doesNotMatch(style, /\.fsdb-detail-icon-slot\{[^}]*width:max-content/)
  assert.doesNotMatch(style, /\.fsdb-detail-icon-slot\{[^}]*justify-content:space-between/)
  const bannerUi = readFileSync(resolve(import.meta.dirname, './page-banner.tsx'), 'utf8')
  assert.match(bannerUi, /fsdb-banner-right/)
  ...
  assert.match(style, /\.fsdb-detail-title-icon\{[^}]*height:78px/)
  assert.match(style, /\.fsdb-detail-icon-slot\{[^}]*margin:-42px/)
  assert.match(style, /\.fsdb-detail-title-icon \.fsdb-record-emoji\{[^}]*font-size:64px/)
  assert.match(style, /\.fsdb-detail-title-icon \.sidebar-mascot,\s*\n\.fsdb-detail-title-icon \.sidebar-mascot svg\{[^}]*width:64px/)
  assert.match(detail, /size=\{64\}/)
  assert.match(detail, /<DetailTitleIcon/)
  assert.match(detail, /recordPreviewEmoji\(selected\)/)
  assert.match(detail, /Icon=\{chrome\?\.Icon\}/)
  assert.match(detail, /<Icon record=\{record\} size=\{64\} \/>/)
...
test('collection header has a layout config control next to the star', () => {
  const menu = readFileSync(resolve(import.meta.dirname, './layout-prefs-menu.tsx'), 'utf8')
  assert.match(browser, /data-testid="fsdb-layout-toggle"/)
  assert.match(browser, /LayoutPrefsMenu/)
  assert.match(browser, /AdjustmentsHorizontalIcon/)
  assert.match(browser, /is-full-width/)
  assert.match(menu, /data-testid=\{`\$\{testPrefix\}-menu`\}/)
  assert.match(menu, /宽屏/)
  assert.match(menu, /展开悬浮/)
  assert.match(menu, /悬浮目录常驻/)
  assert.match(menu, /翻页常驻/)
  assert.match(menu, /persistPagePrefs/)
})
