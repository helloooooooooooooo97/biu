...
test('database extras sit after the record detail, not in the inspector', () => {
  ...
  assert.match(style, /\.fsdb-detail-float-nav\{[^}]*position:absolute/)
  assert.match(style, /\.fsdb-detail-float-nav\{[^}]*right:8px/)
  assert.match(style, /\.fsdb-detail-float-nav\{[^}]*translateY\(-50%\)/)
  assert.match(style, /\.fsdb-detail-float-nav\{[^}]*opacity:0/)
  assert.match(style, /\.fsdb-detail-float-nav::before\{[^}]*left:-12px/)
  assert.doesNotMatch(style, /\.fsdb-detail-float-nav\{[^}]*translate\(-28px,-50%\)/)
