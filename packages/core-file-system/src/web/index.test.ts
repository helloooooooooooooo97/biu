...
test('collection browser renders extra views registered for that path', () => {
  ...
  const page = readFileSync(resolve(import.meta.dirname, './index.tsx'), 'utf8')
  assert.doesNotMatch(page, /registerRowView\('\*', DEFAULT_ROW_VIEW\)/)
  assert.doesNotMatch(page, /DEFAULT_ROW_VIEW/)
  ...
  assert.match(modeMenu, /extraRows\.map/)
  assert.ok(modeMenu.indexOf('VIEW_MODES.map') < modeMenu.indexOf('extraViews.map'))
  assert.ok(modeMenu.indexOf('extraViews.map') < modeMenu.indexOf('extraRows.map'))
  assert.ok(modeMenu.indexOf('extraRows.map') < modeMenu.indexOf('fsdb-mode-plus'))
  assert.match(browser, /askNewPresentation/)
  assert.match(browser, /getPick\(\)\?\.attach/)
  assert.match(browser, /添加呈现方式/)
  assert.doesNotMatch(browser, /modePlusOpen/)
  assert.doesNotMatch(browser, /自定义渲染/)
...
test('tag collect table lives on the record board, not as sidebar views', () => {
  ...
  assert.doesNotMatch(chrome, /panes:/)
  assert.match(detail, /FacetPackEditor/)
...
test('detail facet property sits last, starts collapsed, and the glyph becomes a fold control on hover', () => {
  ...
  const css = readFileSync(resolve(import.meta.dirname, './fsdb-style.ts'), 'utf8')
  assert.match(detail, /collectionPath === FACETS_COLLECTION_PATH/)
  assert.match(detail, /useState\(false\)/)
  assert.match(detail, /collapsible=\{fold\}/)
  assert.match(detail, /packFields/)
  assert.match(detail, /FacetPackEditor/)
  assert.match(row, /收起\$\{label/)
  assert.match(row, /展开\$\{label/)
