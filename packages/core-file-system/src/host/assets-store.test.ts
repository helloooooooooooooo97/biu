...
test('shared assets are content-addressed and reject path escape', async () => {
  ...
  assert.equal((await readFile(join(dir, 'hash', hashedAssetRel(written.name))))[0], 0x89)
...
test('documents overwrite in place and require etag', async () => {
  ...
  assert.equal(await readFile(join(dir, 'name', '画板-ab.json'), 'utf8'), '{"a":1}')
  ...
  assert.equal(await readFile(join(dir, 'name', '画板-ab.json'), 'utf8'), '{"a":2}')
})
