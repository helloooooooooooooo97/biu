...
test('dropInbox and patchInbox edit queued wakes before they are claimed', async () => {
  ...
  await agent.send('挺好的', { wait: false })
  const queued = ctx.agents.listInbox(agent.sessionId)
  assert.equal(queued.length, 1)
  const id = queued[0]!.id
  assert.equal(ctx.agents.patchInbox(agent.sessionId, id, '改一下'), true)
  assert.equal(ctx.agents.listInbox(agent.sessionId)[0]?.text, '改一下')
  assert.equal(ctx.agents.patchInbox(agent.sessionId, id, '   '), false)
  assert.equal(ctx.agents.dropInbox(agent.sessionId, id), true)
  assert.equal(ctx.agents.listInbox(agent.sessionId).length, 0)
  assert.equal(ctx.agents.dropInbox(agent.sessionId, id), false)
  agent.cancel()
})
