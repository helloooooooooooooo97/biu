...
test('page-video is a headless page block with tag grammar', async () => {
  ...
  assert.doesNotMatch(web, /pv-split:hover:after/)
  assert.doesNotMatch(web, /pv-foot-split:hover:after/)
  assert.doesNotMatch(web, /\.pv-src:focus\{[^}]*box-shadow/)
  assert.match(web, /function highlightTimelineScript/)
  assert.match(web, /className="pv-code-highlight"/)
  assert.match(web, /className: 'pv-code-tag'/)
  assert.match(web, /className: 'pv-code-attr'/)
  assert.match(web, /className: 'pv-code-value'/)
  assert.match(web, /highlight\.scrollTop = event\.currentTarget\.scrollTop/)
