...
test('page-video drafts the tag script locally like html source', async () => {
  ...
  assert.match(src, /data-editable=\{editable \? 'true' : undefined\}/)
  ...
  assert.match(src, /highlight\.scrollLeft = event\.currentTarget\.scrollLeft/)
  assert.doesNotMatch(src, /clamp\(22px,4\.4vw,40px\)/)
  assert.doesNotMatch(src, /clamp\(14px,2\.1vw,20px\)/)
  assert.doesNotMatch(src, /const inset = `\$\{project\.padding\}%`/)
  assert.match(src, /function CompileLine/)
  ...
  assert.match(src, /\(captions\.length - 1 - index\) \* 7/)
