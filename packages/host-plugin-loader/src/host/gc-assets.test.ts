...
import { ASSET_GC_CANDIDATE_MS, ASSET_GC_GRACE_MS, gcCasAssets, liveAssetNames } from './gc-assets.ts'
...
test('grace constant and candidate window', () => {
  assert.equal(ASSET_GC_GRACE_MS, 24 * 60 * 60 * 1000)
  assert.equal(ASSET_GC_CANDIDATE_MS, 30 * 24 * 60 * 60 * 1000)
})
