...
test('left sidebar keeps chat and database lists mounted and folds smoothly', () => {
  ...
  assert.match(chat, /<ChatCount[\s\S]*?sidebar-view-switch/)
  assert.match(chat, /<ChatCount count=\{group\.sessions\.length\} \/>[\s\S]*?sidebar-add/)
  assert.match(chat, /\{group\.label\}<\/span>\s*<\/div>\s*<ChatCount count=\{group\.sessions\.length\}/)
...
test('settings and session config floats match search chrome', () => {
  ...
  assert.match(css, /\.settings-float\s*\{[^}]*font-size:\s*14px/)
  assert.match(css, /\.settings-rail\s*\{/)
  assert.match(css, /\.settings-theme-card/)
...
test('settings appearance toggles day and night themes', () => {
  ...
  assert.match(chrome, /日间模式/)
  assert.match(chrome, /夜间模式/)
  assert.match(chrome, /data-theme-card="light"/)
  assert.match(chrome, /data-theme-card="dark"/)
  assert.match(css, /\.settings-theme-preview\.is-light \{[^}]*background-color: #ffffff/)
  assert.match(css, /\.settings-theme-preview\.is-dark \{[^}]*background-color: #191919/)
  assert.match(css, /\.settings-theme-card \{[^}]*border:\s*0/)
  assert.match(css, /\.settings-theme-card\[data-theme-card="light"\] \{[^}]*background: #ffffff/)
  assert.match(css, /\.settings-theme-card\[data-theme-card="dark"\] \{[^}]*background: #191919/)
  assert.doesNotMatch(css, /\.settings-theme-card\[data-theme-card="light"\] \{[^}]*border-color/)
  assert.doesNotMatch(css, /\.settings-theme-card\[data-theme-card="dark"\] \{[^}]*border-color/)
  assert.doesNotMatch(css, /\.settings-theme-preview\.is-light \{[^}]*var\(--dsw/)
  assert.doesNotMatch(css, /\.settings-theme-preview\.is-dark \{[^}]*var\(--dsw/)
  ...
  assert.match(css, /:root \{[^}]*--dsw-icon-active: #5f5e5a/)
  ...
  assert.match(css, /\.sidebar-chat-count \{[^}]*opacity: 0/)
  assert.match(css, /\.sidebar-section-head:hover \.sidebar-chat-count/)
  assert.match(css, /\.sidebar-section-head:hover \.sidebar-chat-count[\s\S]*?min-width: 24px/)
  assert.match(css, /\.chat-session-row:hover \.sidebar-trail-icon[\s\S]*?width: 24px/)
  assert.match(css, /\.chat-session-row:hover \.chat-session-row-star/)
  assert.match(css, /\.chat-session-row-star\.is-on,\s*\n\.chat-session-row-star\.is-on:hover/)
  assert.match(css, /\.app-side-bar \.chat-session-row-star\.is-on svg[\s\S]*?#e8b84a/)
  assert.match(css, /\.chat-session-row-delete,\s*\n\.chat-session-row-star \{[^}]*color:\s*#a6a6a3/s)
  assert.match(css, /\.chat-session-row > \.chat-session-row-delete,\s*\n\.chat-session-row > \.chat-session-row-star/)
  assert.match(css, /\.app-side-bar \.chat-session-row-delete svg,\s*\n\.app-side-bar \.chat-session-row-star svg \{[^}]*color:\s*#a6a6a3/s)
...
test('settings lists an MCP pane for host token and clients', () => {
  ...
  const chrome = readFileSync(resolve(import.meta.dirname, './shell-chrome.tsx'), 'utf8')
  const css = readFileSync(resolve(import.meta.dirname, '../../../../web/style.css'), 'utf8')
  assert.match(shell, /key: 'mcp'/)
  assert.match(shell, /label: 'MCP'/)
  assert.match(shell, /ShellSettingsMcp/)
  ...
  assert.match(css, /\.settings-mcp-card/)
  assert.match(css, /\.settings-mcp-grid/)
})
...
test('settings lists search and pick shortcuts', () => {
  ...
  assert.match(chrome, /<kbd>⌘<\/kbd><kbd>F<\/kbd>/)
  assert.match(chrome, /<kbd>Ctrl<\/kbd><kbd>Q<\/kbd>/)
  assert.match(chrome, /正文查找/)
  assert.match(chrome, /⌘⇧F 仍打开全局搜索/)
  assert.doesNotMatch(chrome, /⌘K/)
  assert.doesNotMatch(chrome, /就地编辑选区/)
  assert.match(chrome, /<kbd>⌘<\/kbd><kbd>L<\/kbd>/)
...
test('settings about states Apache-2.0 license and grok-bot notice', () => {
  ...
  assert.doesNotMatch(chrome, /MIT/)
  assert.doesNotMatch(chrome, /PolyForm/)
