...
test('settings appearance toggles day and night themes', () => {
  ...
  assert.match(css, /\.sidebar-section-head \{[^}]*gap: 2px/)
  assert.match(css, /\.sidebar-group-head \{[^}]*gap: 2px/)
  assert.match(css, /\.chat-session-row \{[^}]*gap: 2px/)
  assert.match(css, /\.sidebar-section-head:hover \.sidebar-chat-count[\s\S]*?min-width: 0/)
  assert.match(css, /\.chat-session-row:hover \.sidebar-trail-icon[\s\S]*?width: 16px/)
