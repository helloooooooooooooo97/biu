...
test('tool panels match the step bar: sidebar fill, no border', () => {
  ...
  assert.doesNotMatch(css, /\.tool-call-head\.is-open\.is-ok::before/)
  assert.doesNotMatch(css, /\.tool-call-head\.is-open\.is-fail::before/)
  assert.match(css, /\.tool-call-chevron\.is-ok\s*\{[^}]*color:\s*#448361/s)
  assert.match(css, /\.tool-call-chevron\.is-fail\s*\{[^}]*color:\s*#c4554d/s)
  assert.match(css, /\.tool-call-title\s*\{[^}]*color:\s*var\(--dsw-sidebar-fg\)/s)
  assert.match(css, /html:not\(\.dark\) \.tool-call-title[\s\S]*color:\s*#5f5e5a/)
  assert.match(css, /\.tool-call-chevron\s*\{[^}]*color:\s*var\(--dsw-icon\)/s)
  assert.match(css, /\.tool-call-inspect\s*\{[^}]*color:\s*#9C9B98/s)
  assert.match(css, /\.tool-call-copy,\s*\.tool-call-copy:hover,\s*\.tool-call-copy\.is-done,\s*\.tool-call-copy svg\s*\{[^}]*color:\s*#9C9B98/s)
  assert.match(css, /\.tool-call-head:hover \.tool-call-chars[\s\S]*display:\s*none/)
  assert.match(css, /:root \{[^}]*--dsw-icon: #91918e/)
  assert.match(css, /html:not\(\.dark\) \[data-testid="content-edits-table"\] \.content-edits-ink[\s\S]*color:\s*#5f5e5a/)
  assert.match(css, /html:not\(\.dark\) \[data-testid="content-edits-table"\] \.content-edits-icon[\s\S]*color:\s*#91918e/)
  assert.match(css, /\.tool-call-tools\s*\{[^}]*width:\s*0/s)
  assert.match(css, /\.tool-call-head:hover \.tool-call-tools[\s\S]*width:\s*auto/)
  assert.match(css, /\.tool-call-head\.is-running::after/)
  assert.match(css, /@keyframes tool-call-text-pulse/)
  assert.match(source, /className: 'is-running'/)
  assert.match(source, /className="tool-call-tools"/)
