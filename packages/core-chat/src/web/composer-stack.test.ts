...
describe('composer dock stacking above sticky user', () => {
  ...
  it('uses facet tones for pick chips and the clear-context control, not the plus', () => {
    ...
    // 加号 / 模型入口 / 芯片 / dock 按钮走中性前景 token，不带任何强调色。
    ...
    expect(css).toMatch(/\.dock-icon-btn\s*\{[^}]*background:\s*var\(--dsw-hover-strong\)/s)
    expect(css).toMatch(/\.dock-icon-btn\.is-active\s*\{[^}]*background:\s*var\(--dsw-hover-strong\)/s)
