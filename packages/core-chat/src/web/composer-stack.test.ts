...
describe('composer dock stacking above sticky user', () => {
  ...
  it('uses facet orange fill on clear-context only when there is history', () => {
    ...
    expect(css).not.toMatch(/\.tool-call-head\.is-open\.is-ok::before/)
    expect(css).not.toMatch(/\.tool-call-head\.is-open\.is-fail::before/)
    expect(css).toMatch(/\.tool-call-tools\s*\{[^}]*width:\s*0/s)
    expect(css).toMatch(/\.tool-call-head:hover \.tool-call-tools[\s\S]*width:\s*auto/)
    expect(css).toMatch(/\.tool-call-inspect\s*\{[^}]*color:\s*#9C9B98/s)
    expect(css).toMatch(/\.tool-call-head:hover \.tool-call-chars[\s\S]*display:\s*none/)
    ...
    expect(css).toMatch(/html\.dark \.chat-pane,\s*\nhtml\.dark \.chat-overlay-panel\s*\{[^}]*--dsw-ok:\s*#1fac60/s)
    expect(css).toMatch(/html\.dark \.traj-usage-ring\.is-cache[\s\S]*color:\s*#1fac60/)
    expect(css).toMatch(/html\.dark \.traj-usage-ring\.is-hist\s*\{[^}]*color:\s*#ff8000/s)
    expect(css).toMatch(/html\.dark \.tool-call-status\.is-fail[\s\S]*color:\s*#ff5949/)
    ...
    expect(usage).not.toMatch(/style=\{\{ color/)
