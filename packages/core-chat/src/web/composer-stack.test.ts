...
describe('composer dock stacking above sticky user', () => {
  ...
  it('uses facet orange fill on clear-context only when there is history', () => {
    ...
    expect(css).toMatch(/html\.dark \.chat-pane,\s*\nhtml\.dark \.chat-overlay-panel\s*\{[^}]*--dsw-ok:\s*#6dffb5/s)
    expect(css).toMatch(/html\.dark \.traj-usage-ring\.is-cache[\s\S]*color:\s*#6dffb5/)
    expect(css).toMatch(/html\.dark \.traj-usage-ring\.is-hist\s*\{[^}]*color:\s*#ffd084/s)
    expect(css).toMatch(/html\.dark \.tool-call-status\.is-fail[\s\S]*color:\s*#ffb4ad/)
