...
test('desktop pack publishes dmg/exe via GitHub Release and ad-hoc macOS signing', async () => {
  ...
  assert.match(source, /stageBuiltinPluginSources/)
  assert.match(source, /join\(hostDir, '\.plugin-dev'\)/)
  assert.match(source, /includes\('node_modules'\)/)
  ...
  assert.match(main, /SHARE_HOST: shareHost/)
  assert.match(main, /availablePort\(/)
  assert.match(main, /shareHost/)
