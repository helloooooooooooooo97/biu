...
test('default sample explains how to edit the block', () => {
  ...
  assert.match(SAMPLE_SCRIPT, /页面里的\|一段视频/)
  assert.match(SAMPLE_SCRIPT, /点全屏\|改里面的字/)
  assert.match(SAMPLE_SCRIPT, /换成你的字\|删掉这段介绍/)
  assert.match(SAMPLE_SCRIPT, /desc="下一步"/)
  ...
  const transitions = project.clips.filter((clip) => clip.src === 'builtin:ad-transition')
  assert.equal(scenes.length, 8)
  assert.equal(transitions.length, 7)
  assert.ok(scenes.every((clip) => clip.kind === 'component' && clip.duration === 3))
  assert.deepEqual(scenes.map((clip) => clip.variant), ['hero', 'split', 'marquee', 'stagger', 'focus', 'code', 'stack', 'finale'])
  assert.deepEqual(scenes.map((clip) => clip.textMotion), ['fade-up', 'slide', 'type', 'reveal', 'scale', 'tracking', 'blur', 'fade'])
  assert.deepEqual(transitions.map((clip) => clip.variant), ['wipe', 'iris', 'split', 'bars', 'flash', 'slide', 'shutter'])
  assert.ok(transitions.every((clip) => clip.kind === 'component' && clip.duration === 26 / project.fps))
  assert.ok(Math.abs((transitions[0]?.start ?? 0) - (3 - 26 / project.fps)) < 1e-9)
  assert.ok(Math.abs((transitions[6]?.start ?? 0) - (21 - 26 / project.fps)) < 1e-9)
  const bgm = project.clips.find((clip) => clip.src === 'assets/bgm.mp3')
  assert.equal(bgm?.kind, 'audio')
  assert.equal(bgm?.start, 0)
  assert.equal(bgm?.duration, 24)
  assert.ok(Math.abs((bgm?.volume ?? 0) - 0.7) < 1e-9)
  assert.match(SAMPLE_SCRIPT, /kind=audio/)
  assert.match(SAMPLE_SCRIPT, /src=assets\/bgm\.mp3/)
  assert.doesNotMatch(SAMPLE_SCRIPT, /demo\/hero\.mp4/)
  assert.equal(projectDuration(project), 24)
  assert.match(dumpScript(project), /variant=marquee/)
  assert.match(dumpScript(project), /motion=tracking/)
