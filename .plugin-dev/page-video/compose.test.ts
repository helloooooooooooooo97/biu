...
test('default sample is a frame-driven React advertisement', () => {
  ...
  assert.ok(Math.abs((transitions[0]?.start ?? 0) - (2 - 26 / project.fps)) < 1e-9)
  assert.ok(Math.abs((transitions[6]?.start ?? 0) - (14 - 26 / project.fps)) < 1e-9)
  const beat = project.clips.find((clip) => clip.src === 'assets/ad-beat.wav')
  assert.equal(beat?.kind, 'audio')
  assert.equal(beat?.start, 0)
  assert.equal(beat?.duration, 16)
  assert.ok(Math.abs((beat?.volume ?? 0) - 0.72) < 1e-9)
  assert.match(SAMPLE_SCRIPT, /kind=audio/)
  assert.match(SAMPLE_SCRIPT, /src=assets\/ad-beat\.wav/)
