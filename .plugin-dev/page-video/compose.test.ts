...
test('default sample is a frame-driven React advertisement', () => {
  ...
  const bgm = project.clips.find((clip) => clip.src === '/api/page-video/assets/bgm.mp3')
  assert.equal(bgm?.kind, 'audio')
  assert.equal(bgm?.start, 0)
  assert.equal(bgm?.duration, 16)
  assert.ok(Math.abs((bgm?.volume ?? 0) - 0.7) < 1e-9)
  assert.match(SAMPLE_SCRIPT, /kind=audio/)
  assert.match(SAMPLE_SCRIPT, /src=\/api\/page-video\/assets\/bgm\.mp3/)
